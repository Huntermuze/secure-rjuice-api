...
class Minecraft:
    ...
    def postToChat(self, msg):
        ...
        # Define the three components of the encrypted message and compute their combination.
        student_ids = '22190497'
        identifier = 'chat.post'
        plaintext = identifier + '/' + msg + '/' + student_ids

        # Compute the ciphertext and the mac.
        ciphertext = self.conn.encryption.encrypt_message(plaintext)
        mac = self.conn.encryption.calculate_mac(ciphertext)

        # Logging to console.
        log('The CIPHERTEXT is: ' + str(ciphertext.decode('utf-8')))
        log('The MAC is: ' + (mac))
        log('The PLAINTEXT is: ' + str(plaintext))

        # Must decode the ciphertext into a string of base64, otherwise will be sent as an an array of bytes.
        self.conn.send(b"decryptionSite", ciphertext.decode('utf-8'), mac)
